...
| 10 | 外部调用熔断、重试、退避 | 防止第三方平台或 LLM 故障拖垮系统 | AI、微信、知乎、X、抖音调用统一 retry/backoff/circuit breaker | 5 | 3 | P1 | 完成 | backend 已新增统一 resilience 层，覆盖 AI service、微信、X、browser-worker、媒体下载 HTTP 调用，并在发布操作层按平台维度保护知乎/抖音等浏览器自动化发布；ai-service LLM 客户端已配置 timeout、max retries 和 stream chunk timeout |
...
- [x] 外部平台调用增加 retry、backoff、timeout 和 circuit breaker。（已完成：backend 统一 resilience 层覆盖 AI service、微信、X、browser-worker、媒体下载 HTTP 调用；发布操作层按平台维度保护微信、知乎、X、抖音；ai-service LLM 客户端支持 timeout、max retries 和 stream chunk timeout。）
...
| [x] | P1 | 外部调用熔断与重试 | 统一 resilience 层已覆盖 AI service、微信、X、browser-worker、媒体下载 HTTP 调用，发布操作层按平台维度保护微信、知乎、X、抖音，ai-service LLM 客户端已配置 timeout/max retries/stream chunk timeout |
