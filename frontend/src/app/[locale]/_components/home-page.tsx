...
import { createHomeStructuredData } from "../_lib/home-seo";

type HomePageProps = {
  locale: string;
};

const workflowSteps = (t: any) => [
  {
...
const featureGroups = (t: any) => [
  {
    title: t("features.group1.title"),
    body: t("features.group1.description"),
  },
  {
    title: t("features.group2.title"),
    body: t("features.group2.description"),
  },
  {
    title: t("features.group3.title"),
    body: t("features.group3.description"),
  },
];

const useCases = (t: any) => [
  t("useCases.case1"),
  t("useCases.case2"),
  t("useCases.case3"),
  t("useCases.case4"),
...
function HomeHeader({ locale, tCommon }: { locale: string; tCommon: any }) {
  return (
    <header className="flex items-center justify-between gap-4">
      <Link href={`/${locale}`} className="flex items-center gap-3">
        <Image src="/icons/mpp.svg" alt="MPP" width={36} height={36} priority />
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3">
          {PLATFORM_TABS.map((platform) => (
            ...
            >
              <Image
                src={platform.icon}
                alt={tCommon(platform.label)}
                width={20}
                height={20}
                className="h-5 w-5"
              />
              {tCommon(platform.label)}
            </span>
          ))}
        </div>
      </div>
    ...
    <div className="max-w-2xl">
      <p
        className={`text-sm font-semibold uppercase tracking-normal ${
          tone === "red" ? "text-[#e5533d]" : "text-[#0f6f78]"
        }`}
      >
        {overline}
      </p>
...
function FeatureCard({ body, title }: { body: string; title: string }) {
  return (
    <article className="rounded-[8px] border border-[#17211c]/12 bg-[#f5f7f1] p-5">
      <CheckCircle2 className="h-5 w-5 text-[#0f8b5f]" />
      <h3 className="mt-5 text-lg font-semibold tracking-normal">{title}</h3>
      <p className="mt-3 text-sm leading-6 text-[#52615a]">{body}</p>
    </article>
  );
}
...
export async function HomePage({ locale }: HomePageProps) {
  ...
  const workflowStepsList = workflowSteps(t);
  const featureGroupsList = featureGroups(t);
  const useCasesList = useCases(t);
  const featureList = workflowStepsList.map((step) => step.title);
  const jsonLd = createHomeStructuredData({
    ...
    <main className="bg-[#f5f7f1] text-[#17211c]">
      ...
      <section className="relative min-h-[88svh] overflow-hidden border-b border-[#17211c]/12">
        ...
        <div className="relative z-10 mx-auto flex min-h-[88svh] w-full max-w-7xl flex-col px-5 py-5 sm:px-8 lg:px-10">
          <HomeHeader locale={locale} tCommon={tCommon} />
          <HeroContent locale={locale} t={t} tCommon={tCommon} />
        </div>
      </section>
      ...
      >
        <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
          <SectionHeading
            overline={t("sections.workflow.overline")}
            title={t("sections.workflow.title")}
          />
          <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {workflowStepsList.map((step, index) => (
              <StepCard
                key={step.title}
      ...
      <section id="platforms" className="bg-white py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 lg:grid-cols-[0.85fr_1.15fr] lg:px-10">
          <div>
            <SectionHeading
              overline={t("sections.platforms.overline")}
              title={t("sections.platforms.title")}
              tone="red"
            />
            <p className="mt-5 text-base leading-7 text-[#52615a]">
              {t("sections.platforms.description")}
            </p>
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            {featureGroupsList.map((feature) => (
              <FeatureCard
                key={feature.title}
                body={feature.body}
                title={feature.title}
